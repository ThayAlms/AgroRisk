...
const memory = globalThis.__agroRiskMemory || {
  telemetry: [], configs: new Map(), logs: [], zones: [], commands: new Map(), locations: new Map(), machines: new Map(),
};
...
async function listMachines() {
  ...
  const rows = await sql()`WITH latest AS (
    ...
    SELECT COALESCE(m.device_id, latest.device_id) AS device_id,
      m.name, m.machine_type, m.model, m.farm_name, m.current_operator, m.notes,
...
async function saveMachine(deviceId, machine) {
  await ensureSchema();
  const normalized = {
    deviceId,
  ...
  const rows = await sql()`INSERT INTO machines (device_id, name, machine_type, model, farm_name, current_operator, notes, active, updated_at)
    VALUES (${deviceId}, ${normalized.name}, ${normalized.type}, ${normalized.model}, ${normalized.farmName}, ${normalized.currentOperator}, ${normalized.notes}, ${normalized.active}, NOW())
    ON CONFLICT (device_id) DO UPDATE SET name=EXCLUDED.name, machine_type=EXCLUDED.machine_type,
      model=EXCLUDED.model, farm_name=EXCLUDED.farm_name, current_operator=EXCLUDED.current_operator,
      notes=EXCLUDED.notes, active=EXCLUDED.active, updated_at=NOW()
    RETURNING device_id, name, machine_type, model, farm_name, current_operator, notes, active`;
...
module.exports = {
  ...
  listTelemetry, addSafetyLog, listSafetyLogs, listDangerZones, saveDangerZone, disableDangerZone,
