...
const memory = globalThis.__agroRiskMemory || {
  telemetry: [], configs: new Map(), logs: [], zones: [], commands: new Map(), locations: new Map(), machines: new Map(), users: new Map(), eventLabels: [], weather: new Map(),
};
...
async function listMachines(customerId = null) {
  ...
  if (!hasDatabase()) {
    ...
    }).filter((machine) => !customerId || machine.customerId === customerId);
  }
