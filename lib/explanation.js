...
function generateExplanation(reading, history = [], machine = {}) {
  ...
  if (!reading || !risk) {
    return {
      ...
      source: { score: null, level: 'SEM_SINAL', evaluatedAt: null },
  ...
  return {
    version: 'natural-language-v1', generatedAt: new Date().toISOString(), machine,
    source: { score: risk.score, level: risk.level, evaluatedAt: risk.evaluatedAt || reading.timestamp },
    quality,
    history: context,
    audit: {
    ...
    audiences: {
      broker: {
        ...
        summary: `${evidenceLine} ${recurrence}`,
      ...
      underwriter: {
        ...
        summary: `${evidenceLine} ${recurrence} Completude da telemetria: ${quality.completenessPercent}%.`,
      ...
      claims: {
        ...
        summary: `${evidenceLine} Os dados descrevem condições observadas pelo equipamento, mas não comprovam isoladamente causa, responsabilidade ou cobertura do evento.`,
