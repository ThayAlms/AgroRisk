...
function buildAlert(deviceId, reading, machineName) {
...
async function notifyOperators(deviceId, reading, machineName = null) {
  const alert = buildAlert(deviceId, reading, machineName);
...
function testAlert(deviceId, channelLabel) {
  return {
    severity: 'warning',
...
async function sendTestAlert(deviceId, recipients, channelName = 'push') {
  const channel = CHANNELS[channelName];
  return deliverThrough(channel, recipients, testAlert(deviceId, channel.label), { track: false });
}
